const REQUIRED: Record<string, string> = {
  ...
  STORAGE_PUBLIC_URL:
    'Public base URL for stored files (e.g. https://pub-xxx.r2.dev)',
};

const OPTIONAL: Record<string, string> = {
  REDIS_URL:
    'Redis connection string — session store + cache disabled without this',
  BETTER_AUTH_URL:
    'Public base URL of this server (default: http://localhost:3000)',
  PORT: 'Port to listen on (default: 3000)',
  ALLOWED_ORIGINS: 'Comma-separated CORS origins (default: all)',
  BOXWALLET_BASE_URL: 'BoxWallet API base URL',
  BOXWALLET_API_KEY:
    'BoxWallet API key — financial splits disabled without this',
  ...
  DISTANCE_PROVIDER_KEY:
    'Google/Mapbox key — falls back to haversine without this',
...
export function checkEnv(): EnvCheckResult {
  ...
  if (
    ...
    missing.push(
      `  ✗ BETTER_AUTH_SECRET  —  must be at least 32 characters (got ${process.env['BETTER_AUTH_SECRET'].length})`,
    );
  }
