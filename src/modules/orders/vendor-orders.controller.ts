...
export class VendorOrdersController {
  ...
  @ApiOperation({
    ...
    description:
      "Returns orders for the authenticated vendor's organisation. Filter by status to build workflow queues.",
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Vendor accepts the order and optionally sets an estimated preparation time. Triggers rider assignment.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @ApiOperation({
    ...
    description:
      'Updates order status to "preparing". Notifies the rider to get ready.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Updates order status to "ready". Notifies the assigned rider to come collect.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @Post(':id/pickup-code')
  @ApiOperation({
    summary: 'Verify rider pickup code',
    description:
      'Validates the 4-digit code the rider presents. Confirms handoff and transitions order to "in_transit".',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @ApiOperation({
    ...
    description: 'Vendor cancels an order before it is dispatched.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @ApiOperation({
    summary: 'Mark an order item as unavailable',
    description:
      'Use when a specific item is out of stock. The customer is notified and can accept a swap or cancel.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @Post(':id/items/swap')
  @ApiOperation({
    summary: 'Propose an item swap',
    description:
      'Suggests a substitute variant for an unavailable item. The customer must accept or reject via POST /orders/:id/items/swap-response.',
  })
  @ApiParam({ name: 'id', example: 'order_01abc' })
