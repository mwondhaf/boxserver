...
export class CustomerOrdersController {
  ...
  @Get()
  @ApiOperation({ summary: 'List my orders', description: 'Returns all orders placed by the authenticated customer, newest first.' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get order detail', description: 'Returns full order with items, status history, and delivery info.' })
  ...
  @Put(':id/cancel')
  @ApiOperation({ summary: 'Cancel an order', description: 'Customer can cancel an order while it is still in pending or confirmed state.' })
  @ApiParam({ name: 'id', example: 'order_01abc' })
  ...
  @ApiOperation({ summary: 'Respond to a vendor item swap proposal', description: 'When a vendor marks an item unavailable and proposes a substitute, the customer can accept or reject it here.' })
  @ApiParam({ name: 'id', example: 'order_01abc' })
