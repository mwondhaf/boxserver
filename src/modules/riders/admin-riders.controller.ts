...
export class AdminRidersController {
  ...
  @Get()
  @ApiOperation({ summary: 'List all riders', description: 'Admin-only. Returns all rider applications with their approval status, ratings, and assigned stages.' })
  ...
  @Put(':id/approve')
  @ApiOperation({ summary: 'Approve or suspend a rider', description: 'Set approve: true to activate a pending rider application. Set approve: false with a suspensionReason to suspend an active rider.' })
  ...
  @Post(':id/stages')
  @ApiOperation({ summary: 'Assign a stage to a rider', description: 'Links the rider to a delivery stage/zone. A rider can have multiple stages; one is marked as primary.' })
  @ApiParam({ name: 'id', example: 'rider_01abc' })
