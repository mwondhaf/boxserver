...
export class AdminFinancialController {
  ...
  @Get('confirmations')
  @ApiOperation({
    summary: 'List wallet split confirmations',
    description:
      'Returns all BoxWallet commission split records. Pending items indicate failed splits that need retry.',
  })
  @ApiResponse({
    status: 200,
    description: 'Array of wallet confirmation records',
  })
  listConfirmations() {
  ...
  @ApiOperation({
    summary: 'Retry a failed commission split',
    description:
      'Re-triggers the BoxWallet API call to split commissions for an order whose initial split failed.',
  })
  ...
  @Get('wallets')
  @ApiOperation({
    summary: 'List vendor BoxWallet mappings',
    description:
      'Returns the mapping between vendor organisations and their BoxWallet wallet IDs.',
  })
...
export class VendorWalletController {
  ...
  @Get()
  @ApiOperation({
    summary: 'Get my vendor wallet',
    description:
      "Returns the BoxWallet wallet details for the authenticated vendor's organisation.",
  })
  @ApiResponse({
    status: 200,
    description: 'Wallet mapping or { message: "No wallet found" }',
  })
