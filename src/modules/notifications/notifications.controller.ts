...
export class NotificationsController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns notifications for the current user. Pass unread=true to fetch only unread items.',
  })
  @ApiQuery({
    name: 'unread',
    required: false,
  ...
  list(@Actor() actor: ActorContext, @Query('unread') unread?: string) {
  ...
  @ApiOperation({
    ...
    description:
      'Returns the number of unread notifications. Use this to power a badge in the UI.',
  })
  ...
  async markRead(@Param('id') id: string, @Actor() actor: ActorContext) {
